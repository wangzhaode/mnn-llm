...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
        ...
        if past is not None or past_key_values is not None:
            ...
            if past is None:
                raise ValueError('past is not None!')
            return {
            ...
            if past is None:
                past = torch.zeros(len(self.transformer.layers), 2, 0, 1, 32, 128)
            return {
                "inputs_embeds": inputs_embeds,
                "past_key_values": past,
                "position_ids": position_ids,
    ...
    def forward(
        ...
        # export embedding or lm_head
        # lm_logits = self.transformer.word_embeddings(inputs_embeds)
        # lm_logits = self.lm_head(inputs_embeds)
